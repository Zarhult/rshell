...
vector<string> parse(string command) {
        ...
        for (int i = 0; i < command.size(); ++i)
        {
            ...
            {
                while(i < command.size() - 1 && command.at(i + 1) != '"' && command.at(i + 1) != '(' && command.at(i + 1) != ')' && command.at(i + 1) != ';' && command.at(i + 1) != ' ') //set i to index of end of command, which precedes either a space, semicolon, or paren (or stop at the end of the command if reached)
                {
                    ++i;
                }
                while (i < command.size() - 2 && command.at(i + 2) == ' ') //get rid of any possible extra spaces
                {
                    command.erase(i + 2, 1);
                }
